			...
			delay(function(){
				$.get(url , function( data ){
					...
					if (productos) {
						...
						toastr.error('Ningun producto encontrado')
						}else{
							$("#listaBusquedaProducto").html("");                        
							for (i = 0; i < productos.length; i++) { 
								//console.log(productos[i].codigo_barra)
								$("#listaBusquedaProducto").append(
									$('<option></option>').val(productos[i].codigo_barra).html(productos[i].descripcion + " disponibles.")
								);
							}
						}	
					}
...
function actualizarTablaArticulos(){
	...
	for(i=0; i < listadoArticulos.length; i++){
		...
		resumen_total_dolar += parseFloat(listadoArticulos[i]["total"] / listadoArticulos[i]["tasa"]);
	}
	$("#tablaResumen").html("");
	$("#tablaResumen").append(
		$('<tr></tr>').html(
	...
	$('#totaldolar').val(resumen_total_dolar.toFixed(2))
}
